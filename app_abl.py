...
def guardar_en_sheets(fila):
    try:
        ...
        # Cargamos desde los Secrets traducidos a TOML
...
if not st.session_state.finalizado:
    ...
    if 'guardado' not in st.session_state:
        fila = [datetime.now().strftime("%Y-%m-%d %H:%M"), st.session_state.nom_eval] + [f"{round(v,1)}%" for v in scores.values()]
        if guardar_en_sheets(fila):
            st.session_state.guardado = True
            st.toast("✅ Datos guardados en Google Sheets")

    fig, ax = plt.subplots(figsize=(10, 4))
    ax.axhspan(66, 100, color='green', alpha=0.1); ax.axhspan(33, 66, color='yellow', alpha=0.1); ax.axhspan(0, 33, color='red', alpha=0.1)
    ax.plot(list(scores.keys()), list(scores.values()), marker='o', color='black', lw=2)
    ax.set_ylim(0, 100); st.pyplot(fig)
    if st.button("🔄 Nuevo"): st.session_state.clear(); st.rerun()
